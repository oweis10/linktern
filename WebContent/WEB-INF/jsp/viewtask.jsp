...
<t:layout>
	...
	<br />
	<form:form action="http://localhost:8080/Link-Tern/edittask" method="post"
		modelAttribute="task">
						...
						<div class="col-md-8">
							<form:select class="form-control" id="obj" path="objective_id">
								...
								<c:forEach items="${objectives }" var="objective">
									<c:choose>
									    <c:when test="${objective.getId() == task.getObjective_id()}">
									        <option selected value="${objective.getId() }">${objective.getName()}</option>
									    </c:when>
									    <c:otherwise>
